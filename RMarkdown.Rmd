...
The R Script is information of how both tasks complete using R programming language capable of anipulating unstructured datasets from text mining to image mining; it is standard practice for maintaining data integrity and ensuring that research is reproducible. There are two plots that are interactive so that can click and rescale. The coding lines that can be seen below are the process of tidy dataset and created plots. The coding process has completed for both tasks. If you want to know more information about how raw datasets choose in PowerStat, you can see it in my GitHub account: https://github.com/Jwinninghoff/NDC-First-Round. However, the final task uses Excel for creating vlookup, as instrcuted, that can be seen in emailed attachment.
...
```
**Figure 1:** The percentage is based on six National Postsecondary Student Aid Study (NPSAS) datasets that show ratio between the deaf respondents who have taken remedial course and who have not.
